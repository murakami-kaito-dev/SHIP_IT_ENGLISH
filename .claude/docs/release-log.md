...
| `v2.0.0` | `6c7ba20` | 2.0.0 (9) | **アップデート申請中**（build 9 紐付け済み・提出ボタン待ち） |

> `v2.0.0` は「次バージョンの配信申請を出す対象」なので、`pubspec.yaml` と
> `AppConstants.appVersion` を **2.0.0 に揃えたコミット**に付ける。
> タグだけ 2.0.0 でアプリの表記が 1.0.0 のままにならないよう注意する。

---

## 2.0.0 (build 9) — 2026-08-17 · App Store アップデート申請（提出待ち）

v1.0.0 公開後の**初のアップデート**。機能面の中身は TestFlight の 1.0.0 (8) と同一で、
バージョン表記を 2.0.0 に揃えた上でビルドし直したもの。

- `pubspec.yaml` `1.0.0+8` → `2.0.0+9` / `AppConstants.appVersion` `1.0.0` → `2.0.0`
  （設定画面フッターとバックアップファイルの `app_version` に反映）
- 内容は build 8 のエントリを参照（ダッキー／クエスト／バッジ／ユニット制／
  クイズ多様化／ホーム画面ウィジェット／デザイン刷新／通知改善 ほか）

**配信**: App Store Connect にアップロード済み（Delivery UUID: `e96d1158-9bac-4bc1-a00b-adec82030798`）
**状態**: `PREPARE_FOR_SUBMISSION`。**最終の「審査に提出」ボタンは未押下**（開発者が押す運用）
**リリース方法**: `AFTER_APPROVAL`（承認と同時に自動公開）
**検証**: `flutter analyze` エラー0

### メタデータの引き継ぎ状況（アップデート申請の注意点）

API で新バージョンを作ると **`promotionalText` は引き継がれず空になる**（既知事象）。
説明文・キーワード・スクリーンショット・審査連絡先・著作権は引き継がれた。

| 項目 | 結果 |
|---|---|
| 説明文 / キーワード / 審査連絡先 / 著作権 | 引き継ぎ済み |
| プロモーションテキスト | **空になったので v1.0 の文言を再設定** |
| このバージョンの新機能（`whatsNew`） | **アップデートでは必須**。新規に作成（371文字） |
| スクリーンショット | v1.0 のものが4枚引き継ぎ（`APP_IPHONE_67`・`COMPLETE`） |

⚠️ **スクリーンショットは v1.0 当時のままでデザイン刷新前**。
`WAITING_FOR_REVIEW` に入ると差し替え不可（409）になるため、更新するなら
`PREPARE_FOR_SUBMISSION` の間に行うこと。
...
**状態**: このビルド自体は審査未提出（App Store で公開中の v1.0 は **build 4**。以降のビルドは TestFlight のみで、審査対象に紐づいていない）
...
## 1.0.0 (build 4) — 2026-08-11 提出 / 2026-08-17 公開 · **App Store 公開済み**

- iOS13対応・全文音声マーク版（コミット `7b86d8c`・タグ `v1.0.0`）
- **このアプリの初回公開版**

**配信**: App Store で配信中
**状態**: **`READY_FOR_SALE`（公開済み）**。2026-08-11 に提出 → 約1週間 `WAITING_FOR_REVIEW` が続いたのち、
2026-08-17 に承認・自動公開（`releaseType = AFTER_APPROVAL` のため承認と同時に公開された）。
審査提出（`reviewSubmissions`）は `COMPLETE`。
**メモ**: 待機が長引いたため Expedited Review を送っている。承認がその効果か通常のキュー順かは不明。
促進リクエストは乱用すると次回以降通りにくくなるため、しばらく使わない方針とする。
