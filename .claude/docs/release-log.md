...
| `v2.0.0` | `6c7ba20` | 2.0.0 (9) | **次の申請対象（アップデート申請）**。アイコン刷新ほか build 7 以降の全変更 |
...
## 2.0.0 (build 9) — 2026-08-17 · バージョン表記のみ更新（未ビルド）

次の申請を **2.0.0** で出すため、`pubspec.yaml` を `1.0.0+8` → `2.0.0+9`、
`AppConstants.appVersion` を `1.0.0` → `2.0.0` に更新（設定画面フッターとバックアップ
ファイルの `app_version` に反映される）。`v2.0.0` タグはこのコミットに付けている。

**中身は build 8 と同一**（下記参照）。バージョン表記を変えただけで、まだビルドも
配信もしていない。申請するには 2.0.0 (9) でビルド・アップロードが必要。
